...
describe('ConnectionService.sendConnectionRequest', () => {
  test('visitor não pode enviar pedido de conexão', async () => {
    const sql = makeSql();
    await expect(ConnectionService.sendConnectionRequest(sql, VISITOR, { username: 'fulano' }, 'cid')).rejects.toMatchObject({
      name: 'ForbiddenError',
    });
  });
  ...
  test('rejeita quando nem username nem telefone são informados', async () => {
    ...
    await expect(ConnectionService.sendConnectionRequest(sql, MEMBER, {}, 'cid')).rejects.toMatchObject({ name: 'ValidationError' });
  });
  ...
  test('rejeita quando username E telefone são informados juntos', async () => {
    ...
    await expect(
      ConnectionService.sendConnectionRequest(sql, MEMBER, { username: 'fulano', phone: '11999999999' }, 'cid')
  ...
  test('alvo inexistente devolve a mesma mensagem genérica de sucesso (não revela nada)', async () => {
    ...
    const res = await ConnectionService.sendConnectionRequest(sql, MEMBER, { username: 'ninguem' }, 'cid');
  ...
  test('pedir conexão consigo mesmo devolve a mesma mensagem genérica, sem inserir nada', async () => {
    ...
    const res = await ConnectionService.sendConnectionRequest(sql, MEMBER, { username: 'eu_mesmo' }, 'cid');
    expect(res.success).toBe(true);
  ...
  test('cria pedido novo quando não existe vínculo anterior', async () => {
    ...
    const res = await ConnectionService.sendConnectionRequest(sql, MEMBER, { username: 'fulano' }, 'cid');
    expect(res.success).toBe(true);
  });
...
describe('ConnectionService.respondToRequest', () => {
  test('decisão inválida lança ValidationError antes de tocar o banco', async () => {
    const sql = makeSql();
    await expect(ConnectionService.respondToRequest(sql, MEMBER, 'conn-1', 'talvez', 'cid')).rejects.toMatchObject({
  ...
  test('aceitar um pedido que não existe/não pertence à pessoa lança ConflictError', async () => {
    ...
    await expect(ConnectionService.respondToRequest(sql, MEMBER, 'conn-1', 'accept', 'cid')).rejects.toMatchObject({
      name: 'ConflictError',
    });
  });
  ...
  test('recusar grava declined_until e responde com sucesso', async () => {
    ...
    const res = await ConnectionService.respondToRequest(sql, MEMBER, 'conn-1', 'decline', 'cid');
    expect(res.success).toBe(true);
  });
});
